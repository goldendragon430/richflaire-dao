...
import { closePeraWalletSignTxnModal } from "@perawallet/connect/dist/modal/peraWalletConnectModalUtils";
...
const Staking = () => {
    ...
    const reward = async () => {
        ...
        let binarySignedTxn = window.algorand.encoding.base64ToMsgpack(
            signedTxns[0]
        );
        ...
        openNotification(
            "success!",
            "Reward transaction sent successfully.",
            true
        );
    ...
    const sendReward = async () => {
        ...
        try {
            let singleTxnGroups = [{ txn: txn, signers: [accountAddress] }];
        ...
        const mnemonic =
            "route pair tourist fun warrior ready collect cloud nasty guess universe transfer agree vendor discover whisper fancy champion lunch black shoot canoe glow able feed"; // Replace this with the mnemonic phrase of the voter's account
    ...
    const stake = async () => {
        ...
        const args = [
            new TextEncoder().encode("stake"),
            intToUint8Array(inputedAmount),
            intToUint8Array(Math.floor(Date.now() / 1000))
        ];
        ...
        try {
            let singleTxnGroups = [{ txn: txn1, signers: [publicKey] }];
            let signedTxns = await peraWallet.signTransaction([
                singleTxnGroups
            ]);
            const { txId } = await client.sendRawTransaction(signedTxns).do();
    ...
    async function fetchStakeAmount() {
        try {
            ...
            console.log("tt", r);
            const localState =
                r["apps-local-states"][r["apps-local-states"].length - 1][
                    "key-value"
                ];
            ...
            const localStateData = accountInfo["apps-local-states"];
    ...
    async function getLocalState(accountAddress, appId) {
        try {
            const accountInfo = await client
                ...
                .do();
            const appLocalState = accountInfo.apps_local_state.find(
                (app) => app.id === appId
            );
            const result = await client
                .tealGetLocalState(accountAddress, appId, btoa("l_staked_amt"))
                .do();
            if (appLocalState) {
    ...
    async function fetchRewardAmount() {
        try {
            ...
            openNotification(
                "success!",
                "Reward amount fetched successfully.",
                true
            );
            ...
            console.error("Error fetching data:", error);
            openNotification("Error!", "Error fetching data: " + error);
        }
    }
    ...
    const openNotification = (msg, desc, success) => {
        api.open({
            message: msg,
            description: desc,
            icon: success ? (
    ...
    return (
        <div>
            ...
            <Row>
                ...
                >
                    <Button type="primary" onClick={fetchStakeAmount}>
                        Fetch Staking State
                    </Button>
                    <div style={{ marginLeft: "20px", marginTop: "10px" }}>
                        Total staking amount: {stakeAmount / 1000000} Algo(
                        {stakeAmount} microAlgo)
                    </div>
                </Col>
                <Col span={8}></Col>
            </Row>
